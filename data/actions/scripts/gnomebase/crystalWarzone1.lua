...
function removerTp(tp)
    ...
    if t then
        ...
        SendMagicEffect(posdotp, CONST_ME_POFF)
    end
end
  ...
  if(getGlobalStorageValue(96974) < 1) then
		setGlobalStorageValue(96974, 1)
			for i = 1, 6 do
				...
				addEvent(SendMagicEffect, i * 20 * 1000, pos, CONST_ME_TELEPORT)
			end
	 end
        
   	 doCreatureSay(cid, "The crystals are damaging!", TALKTYPE_ORANGE_1, false, cid, toPosition)
  	 addEvent(SendMagicEffect, 6*20*1000, posDeathstriker, CONST_ME_TELEPORT)   			
	 ...
	 addEvent(setGlobalStorageValue, 6*20*1000 + 30*60*1000, 96974, 0)
        ...
        PlayerSendCancel(cid, "Wait 30 minutes to start again.")
    end
    return true
end