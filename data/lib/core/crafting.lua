...
function Player:sendVocCraftWindow(config, lastChoice)
    local function buttonCallback(button, choice)	
        ...
        if button.text == "    Recipe    " then
            ...
            local skill = config.system[lastChoice].items[choice.id].skill
            ...
            local storageText = config.system[lastChoice].items[choice.id].storageText or ""
            local details = "In order to craft " .. item .. " you must have " .. skill .. " skill at level " .. skillRequired .. ".\n\n" .. description .. "\n\nYou must collect the following items.\n\nRequired Items:\n"
            
            local craftedCount = config.system[lastChoice].items[choice.id].count or 1
            details = details .. "\n\nCrafted Quantity: " .. craftedCount
            ...
            if requiredStorage == 0 then
                details = details .. "\n\nDiscovered Recipe: Not needed to discover this recipe"
            elseif storageStatus > 0 then
		...
		if button.text == "    Craft    " then
            local itemConfig = config.system[lastChoice].items[choice.id]
            local skill = skillMap[itemConfig.skill] or SKILL_CRAFTING
            local requiredSkill = itemConfig.skill_required
            local playerSkill = self:getSkillLevel(skill)
            if playerSkill < requiredSkill then
                self:sendTextMessage(MESSAGE_INFO_DESCR, "You need a " .. itemConfig.skill .. " skill of at least " .. requiredSkill .. " to craft this item.")
                return true
                end
        ...
        local craftedCount = itemConfig.count or 1
        self:getPosition():sendMagicEffect(176)
        self:getPosition():sendMagicEffect(179)
        local function craftEvent()
            self:addSkillTries(skill, skillTries)
            self:addItem(config.system[lastChoice].items[choice.id].itemID, craftedCount)
            self:sendTextMessage(MESSAGE_INFO_DESCR, "You have crafted x" .. craftedCount .. " [" .. itemName .. "].")
    ...
    for i = 1, #config.system[lastChoice].items do
        local itemName = config.system[lastChoice].items[i].item
        local itemCount = config.system[lastChoice].items[i].count or 1
        local displayName = itemName .. " x" .. itemCount
        window:addChoice(displayName)
    end
