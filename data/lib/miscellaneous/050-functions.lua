...
function getMoneyWeight(money)
	local gold = money
	local demoniac = math.floor(gold/100000000)
	gold = gold - demoniac * 100000000
	local amber = math.floor(gold/1000000)
	gold = gold - amber * 1000000
	local crystal = math.floor(gold / 10000)
	gold = gold - crystal * 10000
	local platinum = math.floor(gold / 100)
	gold = gold - platinum * 100
	return (ItemType(28873):getWeight() * demoniac) + (ItemType(28872):getWeight() * amber) + (ItemType(2160):getWeight() * crystal) + (ItemType(2152):getWeight() * platinum) + (ItemType(2148):getWeight() * gold)
end
...
function isValidMoney(money)
	-- Updated to allow up to 2^64 - 1
    local maxMoneyLimit = 18446744073709551615 -- Maximum for a 64-bit unsigned integer
    return isNumber(money) and money > 0 and money <= maxMoneyLimit
end
