...
local player = Player(cid)
	if item.actionid == 55022 then
		...
		doSendMagicEffect(getCreaturePosition(cid), CONST_ME_TELEPORT)
		PlayerSendTextMessage(cid, 19, "You're entering the realm of dreams.")
	else
		PlayerSendTextMessage(cid, 19, "You do not have level 40+ or missing the Chayenne's magical key.")
	...
	elseif item.actionid == 55024 then
		...
		doSendMagicEffect(getCreaturePosition(cid), CONST_ME_TELEPORT)
		PlayerSendTextMessage(cid, 19, "Goodbye, dream traveller...")
	end
	return TRUE
end