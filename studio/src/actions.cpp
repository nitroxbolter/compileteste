...
bool Actions::registerLuaEvent(Action* event)
{
	...
	if (!action->getItemIdRange().empty()) {
		...
		for (auto id : range) {
			...
			if (!result.second) {
				console::reportError("Actions::registerLuaEvent", "Duplicate registered item with id: " + std::to_string(id) + " in range from id: " + std::to_string(range.front()) + ", to id: " + std::to_string(range.back()));
			}
		}
		return true;
		...
		for (auto id : range) {
			...
			if (!result.second) {
				console::reportError("Actions::registerLuaEvent", "Duplicate registered item with uid: " + std::to_string(id) + " in range from uid: " + std::to_string(range.front()) + ", to uid: " + std::to_string(range.back()));
			}
		}
		return true;
		...
		for (auto id : range) {
			...
			if (!result.second) {
				console::reportError("Actions::registerLuaEvent", "Duplicate registered item with aid: " + std::to_string(id) + " in range from aid: " + std::to_string(range.front()) + ", to aid: " + std::to_string(range.back()));
			}
		}
		return true;
	}

	console::reportError("Actions::registerLuaEvent", "There is no id / aid / uid set for this event");
	return false;
}
