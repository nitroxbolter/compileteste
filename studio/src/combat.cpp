		...
		if (casterPlayer) {
			if (damage.primary.type == COMBAT_PHYSICALDAMAGE) {
				...
				if (strength != 0) {
					...
					damage.primary.value += static_cast<int32_t>(round(damage.primary.value * (strength / 500.)));
				}
			}
			else {
				...
				if (intelligence != 0) {
					// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
					damage.primary.value += static_cast<int32_t>(round(damage.primary.value * (intelligence / 500.)));
				}
			}
			...
			if (damage.secondary.type == COMBAT_PHYSICALDAMAGE) {
				...
				if (strength != 0) {
					// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
					damage.secondary.value += static_cast<int32_t>(round(damage.secondary.value * (strength / 500.)));
				}
			}
			else {
				auto intelligence = casterPlayer->getCharacterStat(CHARSTAT_INTELLIGENCE);
				if (intelligence != 0) {
					// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
					damage.secondary.value += static_cast<int32_t>(round(damage.secondary.value * (intelligence / 500.)));
				}
			}
		}
	...
	for (Creature* creature : toDamageCreatures) {
		...
		if (casterPlayer) {
			if (damageCopy.primary.type == COMBAT_PHYSICALDAMAGE) {
				...
				if (strength != 0) {
					// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
					damageCopy.primary.value += static_cast<int32_t>(round(damageCopy.primary.value * (strength / 500.)));
				}
			}
			else {
				auto intelligence = casterPlayer->getCharacterStat(CHARSTAT_INTELLIGENCE);
				if (intelligence != 0) {
					// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
					damageCopy.primary.value += static_cast<int32_t>(round(damageCopy.primary.value * (intelligence / 500.)));
				}
			}
			...
			if (damageCopy.secondary.type == COMBAT_PHYSICALDAMAGE) {
				...
				if (strength != 0) {
					// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
					damageCopy.secondary.value += static_cast<int32_t>(round(damageCopy.secondary.value * (strength / 500.)));
				}
			}
			else {
				auto intelligence = casterPlayer->getCharacterStat(CHARSTAT_INTELLIGENCE);
				if (intelligence != 0) {
					// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
					damageCopy.secondary.value += static_cast<int32_t>(round(damageCopy.secondary.value * (intelligence / 500.)));
				}
			}
		}
