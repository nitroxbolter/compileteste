...
void Game::saveGameState()
{
	...
	console::print(CONSOLEMESSAGE_TYPE_INFO, "Saving server...", true, "Game::saveGameState");

	if (!saveAccountStorageValues()) {
		console::reportError("Game::saveGameState", "Failed to save account-level storage values.");
	}
...
slots_t getSlotType(const ItemType& it)
{
    ...
    else if (it.weaponType == WeaponType_t::WEAPON_SPELLBOOK) {
        return CONST_SLOT_SPELLBOOK;  // <-- aqui voc  trata o spellbook explicitamente
    }
...
void Game::addMagicEffect(const Position& pos, uint16_t effect, bool renderBelow)
{
	SpectatorVec spectators;
	map.getSpectators(spectators, pos, true, true);
	addMagicEffect(spectators, pos, effect, renderBelow);
}

void Game::addMagicEffect(const SpectatorVec& spectators, const Position& pos, uint16_t effect, bool renderBelow)
{
	for (Creature* spectator : spectators) {
		if (Player* tmpPlayer = spectator->getPlayer()) {
			tmpPlayer->sendMagicEffect(pos, effect, renderBelow);
		}
	}
}
...
void Game::playerSendTooltip(uint32_t playerId, const Position pos, uint16_t spriteId, uint8_t stackPos, bool counterOffer)
{
	Player* player = getPlayerByID(playerId);
	if (!player) {
		return;
	...
	if (counterOffer) {
		...
		thing = internalGetThing(tradePartner, pos, stackPos, spriteId, STACKPOS_LOOK);
		if (!thing) {
			return;
		}
	} else {
		thing = internalGetThing(player, pos, stackPos, spriteId, STACKPOS_LOOK);
		if (!thing) {
			return;
		}
	}
...
bool Game::loadMarketing()
{
	...
	if (resultOffers) {
		do {
			...
			uint8_t rarity = resultOffers->getNumber<uint8_t>("rarity");
			...
			Item* market_item = nullptr;
			if (it.stackable) {
				// Para itens stackable, criar com subType=1 e depois definir a quantidade
				market_item = Item::CreateItem(type, 1);
				if (market_item) {
					market_item->unserializeAttr(propStream);
					market_item->setItemCount(count);
				}
			} else {
				// Para itens não stackable, usar count como subType (charges)
				market_item = Item::CreateItem(type, count);
				if (market_item) {
					market_item->unserializeAttr(propStream);
				}
			}
			...
			//std::cout << "[DEBUG] Loaded market item: " << market_item->getName() << " (ID: " << type << ") with rarity: " << (int)rarity << " from player: " << marketName << std::endl;
			}
		} while (resultOffers->next());
	}

	//std::cout << "[DEBUG] Marketing data loaded successfully!" << std::endl;
...
void Game::addMarketingOffer(std::string marketName, uint32_t uid, Item* item, bool subItemCheck, uint64_t price, uint8_t rarity)
{
    ...
    // Verificar se é item Drown para debug
    bool isDrownItem = false;
    for (auto& itData : item->getRarityAttributes()) {
        if (itData.first == TOOLTIP_ATTRIBUTE_RESISTANCES) {
            IntegerVector types = itData.second.second;
            if (!types.empty()) {
                for (int32_t typeIndex : types) {
                    if (typeIndex == 8) { // COMBAT_DROWNDAMAGE = 8
                        isDrownItem = true;
                        break;
                    }
                }
                ...
                int32_t testValue = item->getAttributeValue(TOOLTIP_ATTRIBUTE_RESISTANCES, 8);
                if (testValue == value) {
                    isDrownItem = true;
                }
            }
            if (isDrownItem) break;
        }
    }
    
    // Obter quantidade correta do item
    uint16_t itemCount = item->getCharges() > 1 ? item->getCharges() : item->getItemCount();
    
    if (isDrownItem) {
        std::cout << "[MARKET DEBUG] Selling Drown item - Player: " << marketName 
                  << ", UID: " << uid << ", Quantity: " << itemCount 
                  << ", Item ID: " << item->getID() << ", Price: " << price << std::endl;
    }

    // Serializa os atributos do item original
    PropWriteStream propWriteStream;
    propWriteStream.clear();
    item->serializeAttr(propWriteStream);

    // Cria o item do market (clone)
    Item* market_item = Item::CreateItem(item->getID(), item->getSubType());
    std::vector<Item*> subItems;

    if (market_item) {
        // Aplica os atributos originais, sem alterar raridade
        PropStream propStream;
        ...
        // Mant�m a quantidade original
        market_item->setItemCount(itemCount);

        // Subitems (se for container)
        if (Container* container = item->getContainer()) {
            std::vector<Item*> checkSubItems = container->getItems(false);
            if (!checkSubItems.empty()) {
                if (Container* market_itemContainer = market_item->getContainer()) {
                    for (Item* subItem : checkSubItems) {
                        g_game.internalAddItem(market_itemContainer, subItem, INDEX_WHEREEVER, 0);
                    }
                }
                ...
                for (Item* subItem : checkSubItems) {
                    ...
                    if (market_subItem) {
                        ...
                        const char* attrSub = propWriteStream.getStream(subAttributesSize);
                        subPropStream.init(attrSub, subAttributesSize);
                        ...
                        dbSuccess = db.executeQuery(fmt::format(
                            "INSERT INTO `player_marketing` (`player_id`, `player_name`, `uid`, `subitem`, `itemtype`, `count`, `price`, `rarity`, `attributes`, `completed`) "
                            "VALUES ({:d}, {:s}, {:d}, 'true', {:d}, {:d}, 0, {:d}, {:s}, 'false') "
                            "ON DUPLICATE KEY UPDATE `attributes`=VALUES(`attributes`)",
                            (player ? player->getGUID() : 1),
                            db.escapeString(marketName),
                            uid,
                            subItem->getID(),
                            subItem->getSubType(),
                            rarity,
                            db.escapeBlob(attributesSub, attributesSubSizeDB)
                        ));
    ...
    // Monta a oferta do market
    market_offer offer;
    offer.item = market_item;
    offer.subItems = subItems;
    offer.price = price;
    offer.rarity = rarity; // mant�m a raridade como dado, sem alterar atributos
    ...
    // Obter quantidade correta para salvar no banco
    uint16_t countToSave = itemCount; // Para stackable, usar getItemCount()
    if (!market_item->isStackable()) {
        countToSave = item->getSubType(); // Para não-stackable, usar subType (charges)
    }
    
    dbSuccess = db.executeQuery(fmt::format(
        "INSERT INTO `player_marketing` (`player_id`, `player_name`, `uid`, `subitem`, `itemtype`, `count`, `price`, `rarity`, `attributes`, `completed`) "
        "VALUES ({:d}, {:s}, {:d}, 'false', {:d}, {:d}, {:d}, {:d}, {:s}, 'false') "
        "ON DUPLICATE KEY UPDATE `count`=VALUES(`count`), `price`=VALUES(`price`), `attributes`=VALUES(`attributes`)",
        (player ? player->getGUID() : 1),
        db.escapeString(marketName),
        uid,
        item->getID(),
    ...
    if (isDrownItem) {
        std::cout << "[MARKET DEBUG] Drown item saved to DB - Count saved: " << countToSave << std::endl;
    }

    // Adiciona � estrutura de marketing
    if (dbSuccess) {
        auto marketIt = marketing.find(marketName);
        if (marketIt != marketing.end()) {
            marketIt->second[uid] = offer;
        } else {
        ...
        // Remove o item original do player
...
bool Game::removeMarketingOffer(std::string marketName, uint32_t uid)
{
    ...
    if (market != marketing.end()) {
        ...
        if (offer != market->second.end()) {
            ...
            // Puxar atributos diretamente do DB
            DBResult_ptr result = db.storeQuery(
                fmt::format("SELECT `attributes` FROM `player_marketing` WHERE `player_name` = {:s} AND `uid` = {:d}", 
                            db.escapeString(marketName), uid)
            );

            if (result) {
                unsigned long attributesSize = 0;  // vari�vel para passar por refer�ncia
                const char* storedAttributes = result->getStream("attributes", attributesSize);

                Item* return_item = Item::CreateItem(item->getID(), item->getSubType());
                if (return_item) {
                    PropStream propStream;
                    propStream.init(storedAttributes, attributesSize);
                    return_item->unserializeAttr(propStream);
                    return_item->setItemCount(item->getCharges() > 1 ? item->getCharges() : item->getItemCount());

                    // Subitems
                    if (return_item->getContainer() && !offer->second.subItems.empty()) {
                        for (Item* subItem : offer->second.subItems) {
                            DBResult_ptr subResult = db.storeQuery(
                                fmt::format("SELECT `attributes` FROM `player_marketing` WHERE `player_name` = {:s} AND `uid` = {:d} AND `subitem` = 'true' AND `itemtype` = {:d}", 
                                            db.escapeString(marketName), uid, subItem->getID())
                            );

                            if (subResult) {
                                unsigned long subAttributesSize = 0;
                                const char* storedSubAttr = subResult->getStream("attributes", subAttributesSize);

                                Item* return_subItem = Item::CreateItem(subItem->getID(), subItem->getSubType());
                                if (return_subItem) {
                                    PropStream subPropStream;
                                    subPropStream.init(storedSubAttr, subAttributesSize);
                    ...
                    // Devolver item ao player ou no ch�o
                    Player* player = getPlayerByName(marketName);
                    if (player) {
    ...
    bool dbSuccess = db.executeQuery(
        fmt::format("UPDATE `player_marketing` SET `completed` = 'true' WHERE `player_name` = {:s} AND `uid` = {:d}", 
                    db.escapeString(marketName), uid)
    );
...
bool Game::buyMarketingOffer(Player* player, std::string marketName, uint32_t uid, uint16_t quant)
{
    ...
    if (market != marketing.end()) {
        ...
        if (offer != market->second.end()) {
            ...
            if (isDrownItem) {
                std::cout << "[MARKET DEBUG] Buying Drown item - Player: " << player->getName() 
                          << ", Seller: " << marketName << ", UID: " << uid 
                          << ", Quantity: " << quant << ", Item ID: " << item->getID() << std::endl;
            }
            ...
            // Puxar atributos diretamente do DB
            DBResult_ptr result = db.storeQuery(
                fmt::format("SELECT `attributes`, `count` FROM `player_marketing` WHERE `player_name` = {:s} AND `uid` = {:d}", 
                            db.escapeString(marketName), uid)
            );
            ...
            const char* storedAttributes = result->getStream("attributes", attributesSize);
            uint16_t dbCount = result->getNumber<uint16_t>("count");

            Item* market_item = Item::CreateItem(item->getID(), item->getSubType());
            if (!market_item) return false;

            PropStream propStream;
            propStream.init(storedAttributes, attributesSize);
            market_item->unserializeAttr(propStream);

            // Ajusta quantidade para compra
            market_item->setItemCount(quant);

            // Subitems
            if (market_item->getContainer() && !offer->second.subItems.empty()) {
                for (Item* subItem : offer->second.subItems) {
                    DBResult_ptr subResult = db.storeQuery(
                        fmt::format("SELECT `attributes` FROM `player_marketing` WHERE `player_name` = {:s} AND `uid` = {:d} AND `subitem` = 'true' AND `itemtype` = {:d}", 
                                    db.escapeString(marketName), uid, subItem->getID())
                    ...
                    g_game.internalAddItem(market_item->getContainer(), return_subItem, INDEX_WHEREEVER, 0);
                }
            }

            // Checa capacidade do player
            if (!player->hasCapacity(market_item, market_item->getItemCount())) {
                ReleaseItem(market_item);
                return false;
            }
            ...
            bool dbSuccess = false;
            if (quant >= dbCount) {
                dbSuccess = db.executeQuery(
                    fmt::format("UPDATE `player_marketing` SET `completed` = 'true' WHERE `player_name` = {:s} AND `uid` = {:d}", 
                                db.escapeString(marketName), uid)
                );
            } else {
                uint16_t remaining = dbCount - quant;
                dbSuccess = db.executeQuery(
                    fmt::format("UPDATE `player_marketing` SET `count` = {:d} WHERE `player_name` = {:s} AND `uid` = {:d}", 
                                remaining, db.escapeString(marketName), uid)
                );
                
                // Atualiza quantidade do item em memória para itens stackable
                if (dbSuccess && item->isStackable()) {
                    uint16_t remainingCount = item->getItemCount() - quant;
                    if (remainingCount > 0) {
                        item->setItemCount(remainingCount);
                    } else {
                        // Se a quantidade restante for 0 ou menor, não deveria acontecer, mas por segurança
                        item->setItemCount(remaining);
                    }
                }
            }

            // Recompensa para vendedor
            uint64_t reward = offer->second.price * quant;
            db.executeQuery(
                fmt::format("INSERT INTO `player_marketing_reward` (`player_name`, `uid`, `reward`, `completed`) VALUES ({:s}, {:d}, {:d}, 'false')", 
                            db.escapeString(marketName), uid, reward)
            );
            ...
            // Remove do mapa se comprado totalmente
            if (dbSuccess && quant >= dbCount) {
                market->second.erase(uid);
            }
            ...
            if (isDrownItem) {
                std::cout << "[MARKET DEBUG] Drown item purchase completed - Buyer: " << player->getName() 
                          << ", Quantity received: " << quant 
                          << ", Remaining in DB: " << (quant >= dbCount ? 0 : (dbCount - quant)) << std::endl;
            }
