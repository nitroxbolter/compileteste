...
class Game
{
	public:
		...
		void playerSendTooltip(uint32_t playerId, const Position pos, uint16_t spriteId, uint8_t stackPos, bool counterOffer);
		...
		void addMagicEffect(const Position& pos, uint16_t effect, bool renderBelow = false);
		static void addMagicEffect(const SpectatorVec& spectators, const Position& pos, uint16_t effect, bool renderBelow = false);
