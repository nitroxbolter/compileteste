...
Attr_ReadValue Item::readAttr(AttrTypes_t attr, PropStream& propStream)
{
	switch (attr) {
		...
		case ATTR_RARITY_ATTRIBUTES: {
			...
			// FIX: Validate rarity when deserializing
			if (rarity > ITEM_RARITY_BRUTAL) {
				std::cout << "[WARNING] Invalid rarity value in item attributes: " << (int)rarity << ", using ITEM_RARITY_COMMON instead" << std::endl;
                                 ...
                                 const Item* item /*= nullptr*/, int32_t subType /*= -1*/, bool addArticle /*= true*/)
{
	...
	if (it.wieldInfo != 0) {
		...
		if (it.wieldInfo & WIELDINFO_MAGLV) {
			...
			s << " magic level " << it.minReqMagicLevel;
		}
...
void Item::getTooltipData(Item* item, uint16_t spriteId, uint16_t count, TooltipDataContainer& tooltipData)
{
	...
	if (it.wieldInfo != 0) {
		...
		if (it.wieldInfo & WIELDINFO_LEVEL) {
			s << " of level " << static_cast<int>(it.minReqLevel);
		}
		...
		if (it.wieldInfo & WIELDINFO_MAGLV) {
			...
			s << " magic level " << static_cast<int>(it.minReqMagicLevel);
		}
...
std::vector<int32_t> Item::getAttributeTypes(ItemTooltipAttributes_t id)
{
	std::vector<int32_t> types;
	std::cout << "[DEBUG getAttributeTypes] Looking for attribute ID: " << (int)id << std::endl;
	std::cout << "[DEBUG getAttributeTypes] Total rarityAttributes entries: " << rarityAttributes.size() << std::endl;
	
	for (auto& itData : rarityAttributes) {
		std::cout << "[DEBUG getAttributeTypes] Checking entry - Attribute ID: " << (int)itData.first << ", Value: " << itData.second.first << ", Types count: " << itData.second.second.size() << std::endl;
		
		if (itData.first == id && !itData.second.second.empty()) {
			std::cout << "[DEBUG getAttributeTypes] Found matching attribute with " << itData.second.second.size() << " types" << std::endl;
			for (int32_t type : itData.second.second) {
				std::cout << "[DEBUG getAttributeTypes] Adding type: " << type << std::endl;
				types.push_back(type);
			}
			// NÃO fazer break aqui - processar TODAS as entradas para este atributo
		}
	}
	
	std::cout << "[DEBUG getAttributeTypes] Returning " << types.size() << " types" << std::endl;
	return types;
}
...
void Item::setRarityLevel(uint32_t rarityLevel)
{
	std::cout << "=================================================================" << std::endl;
	std::cout << "[FUNCAO ITEM.CPP setRarityLevel] : " << rarityLevel << std::endl;
	std::cout << "[DEBUG] setRarityLevel - setado rarity: " << rarityLevel << std::endl;
	
	if (isStackable()) {
		std::cout << "[DEBUG] setRarityLevel - Item � empilh�vel, n�o pode ter raridade" << std::endl;
		return;
	}

	const slots_t slotId = getRaritySlot();
	std::cout << "[DEBUG] setRarityLevel - Item slot ID: " << (int)slotId << std::endl;
	
	if (slotId == CONST_SLOT_WHEREEVER) {
		std::cout << "[DEBUG] setRarityLevel - Slot do item � WHEREEVER, n�o pode ter raridade" << std::endl;
		return;
	}

	if (!canHoldRarityLevel(slotId)) {
		std::cout << "[DEBUG] setRarityLevel - Item n�o pode ter n�vel de raridade" << std::endl;
		return;
	}

	ItemRarity_t id = static_cast<ItemRarity_t>(rarityLevel);
	
	if (id > ITEM_RARITY_BRUTAL) {
		std::cout << "[WARNING] setRarityLevel - Valor de raridade invalido: " << (int)id << ", using ITEM_RARITY_COMMON instead" << std::endl;
		id = ITEM_RARITY_COMMON;
	}
	...
	if (ItemRarityAttributes::getInstance()->setRandomAttributes(id, slotId, &rarityAttributes)) {
		// Set the rarity id only if the attributes have been set correctly
		rarityId = id;
		std::cout << "[DEBUG] setRarityLevel - ID de raridade definido com sucesso para: " << (int)rarityId << std::endl;
		std::cout << " ================================================================== " << std::endl;
		
		// Rarity is already set via rarityId, no need for custom attribute
