...
void ScriptEnvironment::insertItem(uint32_t uid, Item* item)
{
	...
	if (!result.second) {
		std::string location = "ScriptEnvironment::addThing";
		console::reportError(location, "Lua Script Error: Thing uid already taken.");
	}
}
...
void LuaScriptInterface::reportError(const char* function, const std::string& error_desc, lua_State* L /*= nullptr*/, bool stack_trace /*= false*/)
{
	...
	std::string location = "LuaScriptInterface::reportError";
	std::string errorMsg = "Lua Script Error: ";

	if (scriptInterface) {
		errorMsg += "[" + scriptInterface->getInterfaceName() + "] ";

		if (timerEvent) {
			errorMsg += "in a timer event called from: ";
		}

		if (callbackId) {
			errorMsg += "in callback: " + scriptInterface->getFileById(callbackId) + " ";
		}

		errorMsg += scriptInterface->getFileById(scriptId) + " ";
	}

	if (function) {
		errorMsg += std::string(function) + "(). ";
	}

	if (L && stack_trace) {
		errorMsg += getStackTrace(L, error_desc);
	} else {
		errorMsg += error_desc;
	}

	console::reportError(location, errorMsg);
}
...
// Enums
	...
	REGISTER_ENUM(L, CONSOLEMESSAGE_TYPE_INFO);
	REGISTER_ENUM(L, CONSOLEMESSAGE_TYPE_STARTUP);
	REGISTER_ENUM(L, CONSOLEMESSAGE_TYPE_STARTUP_SPECIAL);
	REGISTER_ENUM(L, CONSOLEMESSAGE_TYPE_WARNING);
	REGISTER_ENUM(L, CONSOLEMESSAGE_TYPE_ERROR);
	REGISTER_ENUM(L, CONSOLEMESSAGE_TYPE_BROADCAST);
	...
	REGISTER_ENUM(L, WEAPON_SPELLBOOK);
...
void LuaScriptInterface::registerFunctions()
{
	...
	//Rarity
	registerMethod("Item", "getRarityLevel", LuaScriptInterface::luaItemGetRarityLevel);
	registerMethod("Item", "getRarity", LuaScriptInterface::luaItemGetRarity);
	registerMethod("Item", "getRarityAttributes", LuaScriptInterface::luaItemGetRarityAttributes);
	registerMethod("Item", "setRarityLevel", LuaScriptInterface::luaItemSetRarityLevel);
	registerMethod("Item", "setRandomRarity", LuaScriptInterface::luaItemSetRandomRarity);
	...
	registerMethod("Player", "getTotalIncrementCoins", LuaScriptInterface::luaPlayerGetTotalIncrementCoins);
	...
	registerMethod("Spell", "renderBelow", LuaScriptInterface::luaSpellRenderBelow);
...
int LuaScriptInterface::luaGameLoadMap(lua_State* L)
{
	...
	g_dispatcher.addTask(createTask([path]() {
		try {
			...
			std::string location = "LuaScriptInterface::luaGameLoadMap";
			console::reportError(location, "Failed to load map: " + std::string(e.what()));
		}
	}));
	return 0;
}
...
{
	// position:sendMagicEffect(magicEffect[, player = nullptr][, renderBelow = false])
	// ou position:sendMagicEffect(magicEffect, renderBelow)
	SpectatorVec spectators;
	bool renderBelow = false;
	int top = lua_gettop(L);
	
	if (top >= 3) {
		// Verificar se o segundo parâmetro (após magicEffect) é um boolean
		// Se for, então é renderBelow e não há player
		if (lua_isboolean(L, 3)) {
			renderBelow = getBoolean(L, 3);
		} else {
			// Caso contrário, é um player
			Player* player = getPlayer(L, 3);
			if (player) {
				spectators.emplace_back(player);
			}
			// Verificar se há um quarto parâmetro (renderBelow)
			if (top >= 4 && lua_isboolean(L, 4)) {
				renderBelow = getBoolean(L, 4);
			}
		}
	}
	...
	const Position& position = getPosition(L, 1);
	if (!spectators.empty()) {
		Game::addMagicEffect(spectators, position, magicEffect, renderBelow);
	} else {
		g_game.addMagicEffect(position, magicEffect, renderBelow);
	}
...
//Rarity
int LuaScriptInterface::luaItemGetRarityLevel(lua_State* L)
{
	// item:getRarityLevel()
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		lua_pushnumber(L, item->getRarityLevel());
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int LuaScriptInterface::luaItemGetRarity(lua_State* L)
{
	// item:getRarity()
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
...
int LuaScriptInterface::luaItemGetRarityAttributes(lua_State* L)
{
	// item:getRarityAttributes()
	Item* item = getUserdata<Item>(L, 1);
	if (!item) {
		lua_pushnil(L);
		return 1;
	...
	const auto& rarityAttributes = item->getRarityAttributes();
	if (rarityAttributes.empty()) {
		lua_newtable(L);
		return 1;
	}

	// Criar tabela Lua com os atributos de raridade
	lua_newtable(L);
	int index = 1;

	for (const auto& itData : rarityAttributes) {
		ItemTooltipAttributes_t attributeId = itData.first;
		int32_t value = itData.second.first;
		const IntegerVector& types = itData.second.second;

		// Criar tabela para cada atributo
		lua_newtable(L);
		
		// Adicionar ID do atributo
		lua_pushnumber(L, attributeId);
		lua_setfield(L, -2, "id");
...
int LuaScriptInterface::luaItemSetRarityLevel(lua_State* L)
{
	// item:setRarityLevel(rarityLevel)
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		uint32_t rarityLevel = getNumber<uint32_t>(L, 2);
		item->setRarityLevel(rarityLevel);
	} else {
		lua_pushnil(L);
	}
...
int LuaScriptInterface::luaItemSetRandomRarity(lua_State* L)
{
	// item:setRandomRarity(fromMonster)
	Item* item = getUserdata<Item>(L, 1);
	if (item) {
		bool fromMonster = getBoolean(L, 2, false);
		item->setRarityLevel(item->getPosition(), fromMonster);
	} else {
		lua_pushnil(L);
	}
	return 1;
...
int32_t LuaScriptInterface::luaPlayerGetTotalIncrementCoins(lua_State* L)
{
    // Obtem o player do primeiro argumento
    Player* player = getUserdata<Player>(L, 1);
    if (!player) {
        lua_pushnil(L); // retorna nil se n o for v lido
        return 1;
    }

    lua_pushnumber(L, player->getTotalIncrementCoins()); // retorna o valor
    return 1;
}
	...
	if (monsterType) {
		if (lua_gettop(L) == 1) {
			...
			if (immunity == "physical") {
				...
				std::string location = "LuaScriptInterface::luaMonsterTypeDamageImmunities";
				console::reportWarning(location, "Unknown immunity name " + immunity + " for monster: " + monsterType->name);
				lua_pushnil(L);
			}
		}
	} else {
	...
	if (monsterType) {
		...
		if (spell) {
			...
			if (g_monsters.deserializeSpell(spell, sb, monsterType->name)) {
				...
				std::string location = "LuaScriptInterface::luaMonsterTypeAddAttack";
				console::reportWarning(location, "Cant load spell " + spell->name + " for monster: " + monsterType->name);
			}
		} else {
			lua_pushnil(L);
		}
...
int LuaScriptInterface::luaSpellRenderBelow(lua_State* L)
{
	// spell:renderBelow(bool)
	Spell* spell = getUserdata<Spell>(L, 1);
	if (spell) {
		if (lua_gettop(L) == 1) {
			pushBoolean(L, spell->getRenderBelow());
		} else {
			spell->setRenderBelow(getBoolean(L, 2));
			pushBoolean(L, true);
		}
	} else {
		lua_pushnil(L);
...
int LuaScriptInterface::luaGameGetMarketingOffers(lua_State* L)
{
	...
	for (auto offer : market) {
		// Corrige valores de raridade inválidos (valores muito altos)
		uint8_t correctedRarity = offer.second.rarity;
		if (offer.second.rarity > 5) {
			correctedRarity = ((offer.second.rarity - 1) % 5) + 1;
		}
		if (offer.second.item) {
			
			// Extrair atributos de raridade diretamente do rarityAttributes multimap
			std::vector<std::pair<int, int>> rarityAttributes; // {id, value}
			
			// Acessar diretamente o rarityAttributes multimap do item
			// Como é um membro privado, vamos usar uma abordagem diferente
			// Vamos iterar pelos atributos conhecidos usando getAttributeValue
			// Baseado no rarityAttributes.lua - TODOS os atributos disponíveis no sistema
			std::vector<std::pair<ItemTooltipAttributes_t, int>> knownAttributes = {
				// Atributos de Ataque Elemental
			...
			// Estrutura para armazenar atributos individuais com seus tipos
			struct IndividualAttribute {
				int displayId;
				int32_t value;
				IntegerVector types;
			};
			std::vector<IndividualAttribute> individualAttributes;
			
			// Processar cada entrada de atributo individualmente para preservar valores corretos
			for (auto& itData : offer.second.item->getRarityAttributes()) {
				ItemTooltipAttributes_t attributeId = itData.first;
				int32_t value = itData.second.first;
				IntegerVector types = itData.second.second;
				
				printf("[CPP DEBUG] Processing attribute: ID=%d, Value=%d, Types count=%zu\n", 
					attributeId, value, types.size());
				
				// Debug: mostrar os tipos se existirem
				if (!types.empty()) {
					printf("[CPP DEBUG] Types for attribute %d: ", attributeId);
					for (size_t i = 0; i < types.size(); ++i) {
						printf("%d ", types[i]);
					}
					printf("\n");
				...
				// Verificar se é um atributo conhecido
				bool isKnownAttribute = false;
				int attrDisplayId = 0;
				for (const auto& knownAttr : knownAttributes) {
					if (knownAttr.first == attributeId) {
						isKnownAttribute = true;
						attrDisplayId = knownAttr.second;
						break;
					}
				}
				
				if (isKnownAttribute && value > 0) {
					// Para atributos com tipos, criar uma entrada separada para cada tipo
					if (!types.empty() && (attributeId == TOOLTIP_ATTRIBUTE_RESISTANCES || attributeId == TOOLTIP_ATTRIBUTE_STATS || 
						attributeId == TOOLTIP_ATTRIBUTE_SKILL || attributeId == TOOLTIP_ATTRIBUTE_INCREMENTS)) {
						
						// Para cada tipo, criar uma entrada separada
						for (size_t i = 0; i < types.size(); ++i) {
							IndividualAttribute individualAttr;
							individualAttr.displayId = attrDisplayId;
							individualAttr.value = value;
							individualAttr.types.push_back(types[i]);
							individualAttributes.push_back(individualAttr);
						}
					} else {
						// Para atributos sem tipos
						IndividualAttribute individualAttr;
						individualAttr.displayId = attrDisplayId;
						individualAttr.value = value;
						individualAttributes.push_back(individualAttr);
			...
			setField(L, "rarity", correctedRarity);
			setField(L, "look", "You see " + offer.second.item->getDescription(0));
			
			// Adicionar rarityAttributes se houver algum
			if (!individualAttributes.empty()) {
				lua_createtable(L, individualAttributes.size(), 0);
				int attrIndex = 0;
				for (const auto& attr : individualAttributes) {
					lua_createtable(L, 0, 3); // id, value, types
					setField(L, "id", attr.displayId);     // ID do atributo (38, 34, 36, etc.)
					setField(L, "value", attr.value); // Valor do atributo
					
					// Adicionar tipos se existirem
					if (!attr.types.empty()) {
						lua_createtable(L, attr.types.size(), 0);
						for (size_t i = 0; i < attr.types.size(); ++i) {
							lua_pushinteger(L, attr.types[i]);
							lua_rawseti(L, -2, i + 1); // Lua arrays começam em 1
						}
						lua_setfield(L, -2, "types");
					}
					
					lua_rawseti(L, -2, ++attrIndex);
				}
				lua_setfield(L, -2, "rarityAttributes");
			}
...
{
    int top = lua_gettop(L);
    std::cout << "[DEBUG] Lua stack size: " << top << std::endl;

    std::string marketName = getString(L, 1);          // Nome do jogador/loja
    uint32_t uid = getNumber<uint32_t>(L, 2);         // UID do item
    Item* item = getUserdata<Item>(L, 3);             // Item
    bool subItemCheck = getBoolean(L, 4);             // Booleano de subitem
    uint64_t price = getNumber<uint64_t>(L, 5);       // Pre�o
    double rawRarity = lua_tonumber(L, 6);            // Agora pega o �ndice 6, que � a raridade
    uint8_t rarity = static_cast<uint8_t>(rawRarity);

    std::cout << "[DEBUG] luaGameAddMarketingOffer - Raw rarity from Lua (double): " << rawRarity << std::endl;
    std::cout << "[DEBUG] luaGameAddMarketingOffer - Converted rarity (uint8_t): " << (uint16_t)rarity << std::endl;

    // Valida��o adicional
    if (rawRarity != static_cast<double>(rarity)) {
        std::cout << "[WARNING] luaGameAddMarketingOffer - Conversion loss detected! Raw: "
                  << rawRarity << " -> Converted: " << (uint16_t)rarity << std::endl;
    }
    ...
    // Chamada para a fun��o que adiciona o item no marketing
    g_game.addMarketingOffer(marketName, uid, item, subItemCheck, price, rarity);
