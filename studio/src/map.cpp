...
{
	std::string location = "Map::loadMap";
	IOMap loader;
	if (!loader.loadMap(this, identifier)) {
		console::reportError(location, loader.getLastErrorString());
		return false;
	}

	if (!IOMap::loadSpawns(this)) {
		console::reportWarning(location, "Failed to load spawn data.");
	}

	if (loadHouses) {
		if (!IOMap::loadHouses(this)) {
			console::reportWarning(location, "Failed to load house data.");
		}
