...
{
	std::string location = "Monsters::deserializeSpell";
	std::string name;
	...
	if (isScripted) {
		...
		if (tmpName == "melee") {
			...
			console::reportWarning(location, description + " - you cannot reduce a creatures speed below -1000 (100%)");
			minSpeedChange = -1000;
		}
		...
		for (auto attributeNode : node.children()) {
			if ((attr = attributeNode.attribute("key"))) {
				...
				if (strcasecmp(value, "shooteffect") == 0) {
					...
					console::reportWarning(location, description + " - Unknown shootEffect: " + attr.as_string());
				}
					}
					...
					console::reportWarning(location, description + " - Unknown areaEffect: " + attr.as_string());
				}
					}
				} else {
				console::reportWarning(location, "Effect type \"" + std::string(attr.as_string()) + "\" does not exist.");
			}
			}
		}
	}
...
bool Monsters::deserializeSpell(MonsterSpell* spell, spellBlock_t& sb, const std::string& description)
{
	...
	if (spell->isScripted) {
		...
		if (!combatSpellPtr->loadScript("data/" + g_spells->getScriptBaseName() + "/scripts/" + spell->scriptName)) {
			std::string location = "Monsters::deserializeSpell";
			console::reportError(location, "Cannot find file: data/" + g_spells->getScriptBaseName() + "/scripts/" + spell->scriptName);
			return false;
		}
	...
	if ((node = monsterNode.child("script"))) {
		for (auto eventNode : node.children()) {
			if ((attr = eventNode.attribute("name"))) {
				...
				std::string location = "Monsters::loadMonster";
