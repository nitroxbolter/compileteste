...
{
	std::string location = "Mounts::loadFromXml";
	...
	for (auto mountNode : doc.child("mounts").children()) {
		...
		if (nodeId == 0 || nodeId > std::numeric_limits<uint8_t>::max()) {
			console::reportWarning(location, fmt::format("Mount id '{}' is not within 1 and 255 range", nodeId));
			continue;
		}

		if (getMountByID(nodeId)) {
			console::reportWarning(location, fmt::format("Duplicate mount with id: {}", nodeId));
			continue;
		}
