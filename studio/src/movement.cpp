	...
	if (it == map.end()) {
		...
		for (MoveEvent& existingMoveEvent : moveEventList) {
			if (existingMoveEvent.getSlot() == moveEvent.getSlot()) {
				std::string location = "MoveEvents::addEvent";
				console::reportWarning(location, "Duplicate move event found: " + std::to_string(id));
			}
		}
...
void MoveEvents::addEvent(MoveEvent moveEvent, const Position& pos, MovePosListMap& map)
{
	...
	if (it == map.end()) {
		...
		if (!moveEventList.empty()) {
			...
			ss << pos;
			console::reportWarning(location, "Duplicate move event found: " + ss.str());
		}
...
std::string MoveEvent::getScriptEventName() const
{
	switch (eventType) {
		...
		default:
			std::string location = "MoveEvent::getScriptEventName";
			console::reportError(location, "Invalid event type");
			return std::string();
	}
}
...
bool MoveEvent::configureEvent(const pugi::xml_node& node)
{
	...
	if (!eventAttr) {
		std::string location = "MoveEvent::configureMoveEvent";
		console::reportError(location, "Missing event");
		return false;
	}
	...
	if (tmpStr == "stepin") {
		...
		std::string location = "MoveEvent::configureMoveEvent";
		console::reportError(location, "No valid event name " + std::string(eventAttr.as_string()));
		return false;
	}
