...
bool Outfits::loadFromXml()
{
	...
	for (auto outfitNode : doc.child("outfits").children()) {
		...
		if (!(attr = outfitNode.attribute("type"))) {
			std::string location = "Outfits::loadFromXml";
			console::reportWarning(location, "Missing outfit type.");
			continue;
		}
		...
		if (type > PLAYERSEX_LAST) {
			std::string location = "Outfits::loadFromXml";
			console::reportWarning(location, "Invalid outfit type " + std::to_string(type) + ".");
			continue;
		}
		...
		if (!lookTypeAttribute) {
			std::string location = "Outfits::loadFromXml";
			console::reportWarning(location, "Missing looktype on outfit.");
			continue;
		}
