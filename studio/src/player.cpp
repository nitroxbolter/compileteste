...
#include <fmt/format.h>
#include <cmath> // <- agora você pode usar std::round normalmente
...
uint32_t Player::getAttackSpeed() const
{
	...
	// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
	ret = ret - (ret * getCharacterStat(CHARSTAT_DEXTERITY) / 500.0);
	return ret;
}
...
void Player::addStorageValue(const uint32_t key, const int32_t value, const bool isLogin/* = false*/)
{
	if (IS_IN_KEYRANGE(key, RESERVED_RANGE)) {
		if (IS_IN_KEYRANGE(key, OUTFITS_RANGE)) {
			...
			console::reportWarning("Player::addStorageValue", "Unknown reserved key: " + std::to_string(key) + " player: " + getName());
			return;
		}
	}
...
void Player::onCreatureAppear(Creature* creature, bool isLogin)
{
	...
	if (isLogin && creature == this) {
		...
		if (g_config.getBoolean(ConfigManager::PLAYER_CONSOLE_LOGS)) {
			std::string location = "Player::onCreatureAppear";
			console::print(CONSOLEMESSAGE_TYPE_INFO, fmt::format("{} has logged in.", name));
		}
...
void Player::onRemoveCreature(Creature* creature, bool isLogout)
{
	...
	if (creature == this) {
		...
		if (!saved) {
			std::string location = "Player::onRemoveCreature";
			console::reportError(location, fmt::format("Error while saving player: {}", getName()));
		}
	}
}
		...
		case CONST_SLOT_LEFT: {
			if (slotPosition & SLOTP_LEFT) {
				if (!g_config.getBoolean(ConfigManager::CLASSIC_EQUIPMENT_SLOTS)) {
					...
					// Modificação: Adicionando WEAPON_SPELLBOOK à verificação
					if (type == WEAPON_NONE || type == WEAPON_SHIELD || type == WEAPON_AMMO || type == WEAPON_SPELLBOOK) {
						ret = RETURNVALUE_CANNOTBEDRESSED;
					...
					if (rightItem->getSlotPosition() & SLOTP_TWO_HAND) {
						...
						ret = RETURNVALUE_CANONLYUSEONESHIELD;
					           ...
					           rightType == WEAPON_SHIELD || rightType == WEAPON_AMMO || // Removido WEAPON_SPELLBOOK
...
void Player::gainExperience(uint64_t gainExp, Creature* source)
{
	...
	// Convertemos o gainExp para double para aplicar o bônus
	double modExp = static_cast<double>(gainExp);
	onGainExperience(modExp, false);
	addExperience(source, static_cast<uint64_t>(std::round(modExp)), true);
}

void Player::onGainExperience(double& gainExp, bool)
{
	int32_t bonus = 0;
	...
	for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; ++i) {
		...
		int32_t itemBonus = item->getAttributeValue(TOOLTIP_ATTRIBUTE_EXPERIENCE);
		if (itemBonus > 0) {
			bonus += itemBonus;
		}
	}
...
{
	double modExp = static_cast<double>(gainExp);
	onGainExperience(modExp, true); // true = é party

	gainExperience(static_cast<uint64_t>(modExp), source);
}
...
void Player::onGainExperience(uint64_t gainExp, Creature* target)
{
    ...
    // Aplica bônus de experiência de itens equipados
    int32_t bonus = getItemAttributeValue(TOOLTIP_ATTRIBUTE_EXPERIENCE);
    if (bonus > 0) {
        gainExp += gainExp * bonus / 100; // Aplica o bônus
    }

    if (target && !target->getPlayer() && party && party->isSharedExperienceActive() && party->isSharedExperienceEnabled()) {
        // Aplica o bônus ao compartilhar a experiência
        uint64_t sharedExp = gainExp; // Armazena a experiência a ser compartilhada
        party->shareExperience(sharedExp, target);
        // O ganho de exp será processado via onGainSharedExperience
        return;
    }

    // Aplica o bônus de experiência normal (chamará a função com double&)
    Creature::onGainExperience(gainExp, target);

    // Aplica a experiência final
    gainExperience(gainExp, target);
}

int32_t Player::getTotalIncrementCoins() const {
    int32_t bonus = 0;
    for (int32_t i = CONST_SLOT_FIRST; i <= CONST_SLOT_LAST; ++i) {
        Item* item = inventory[i];
        if (!item) {
        ...
        int32_t itemBonus = item->getAttributeValue(TOOLTIP_ATTRIBUTE_INCREMENT_COINS);
        if (itemBonus > 0) {
            bonus += itemBonus;
        }
    }
