...
class Player final : public Creature, public Cylinder
{
	public:
		...
		int32_t getMaxHealth() const override {
			...
			uint32_t vitHealth = static_cast<uint32_t>(healthMax * (charStats[CHARSTAT_VITALITY] / 500.f));
		...
		uint32_t getMaxMana() const {
			// 5 pontos = 1% de bônus (divide por 500 ao invés de 100)
			uint32_t sprMana = static_cast<uint32_t>(manaMax * (charStats[CHARSTAT_SPIRIT] / 500.f));
		...
		uint64_t getGainedExperience(Creature* attacker) const;
		int32_t getTotalIncrementCoins() const;
		...
		void onGainExperience(uint64_t gainExp, Creature* target);
		void onGainSharedExperience(uint64_t gainExp, Creature* source);
		void onGainExperience(double& gainExp, bool inParty);
		void onSharedExperience(uint64_t& gainExp);
		...
		void sendMagicEffect(const Position& pos, uint16_t type, bool renderBelow = false) const {
			if (client) {
				client->sendMagicEffect(pos, type, renderBelow);
			}
		}
		void sendPing();
