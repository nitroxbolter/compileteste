...
void ProtocolGame::sendTradeItemRequest(const std::string& traderName, const Item* item, bool ack)
{
	...
	auto sendItemPosition = [&msg](Player* player, const Item* item) {
		const auto parent = item->getParent();
		if (parent) {
			if (parent->getItem()) {
				msg.add<uint16_t>(0xFFFF); // Container
				msg.add<uint16_t>(0x40);
				msg.addByte(parent->getThingIndex(item));
			} else if (parent == player) {
				msg.add<uint16_t>(0xFFFF); // Container
				msg.add<uint16_t>(player->getThingIndex(item));
				msg.addByte(0);
			} else if (parent == player->tradePartner) {
				msg.add<uint16_t>(0xFFFF); // Container
				msg.add<uint16_t>(player->tradePartner->getThingIndex(item));
				msg.addByte(0);
			} else {
	...
	if (const Container* tradeContainer = item->getContainer()) {
		...
		for (const Item* listItem : itemList) {
			...
			sendItemPosition(player, listItem);
		}
	} else {
		msg.addByte(0x01);
		msg.addItem(item);
		sendItemPosition(player, item);
	}
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendMagicEffect(const Position& pos, uint16_t type, bool renderBelow)
{
	if (!canSee(pos)) {
		return;
	}

	// Se renderBelow estiver ativo e for cliente OTCv8+, usar AttachedEffect
	if (renderBelow && otclientV8) {
		sendAttachedEffect(pos, type, true);
		return;
	}
...
void ProtocolGame::sendAttachedEffect(const Position& pos, uint16_t effectId, bool renderBelow)
{
	if (!canSee(pos) || !otclientV8) {
		return;
	}

	// Enviar via opcode estendido (0x32) para AttachedEffect
	NetworkMessage msg;
	msg.addByte(0x32); // Extended opcode
	msg.addByte(0x02); // Opcode para AttachedEffect
	msg.addPosition(pos);
	msg.add<uint16_t>(effectId);
	msg.addByte(renderBelow ? 1 : 0); // Flag renderBelow
	writeToOutputBuffer(msg);
}
...
void ProtocolGame::sendAnimatedText(const std::string& message, const Position& pos, TextColor_t color)
{
    ...
    NetworkMessage msg;
    msg.addByte(0x84); // Opcode de texto animado
    msg.addPosition(pos);
    msg.addByte(color);
    msg.addString(message);
    writeToOutputBuffer(msg);
...
void ProtocolGame::parseTooltip(NetworkMessage& msg)
{
	switch (msg.getByte())
	{
		...
		{
			// Real inventory item
			bool counterOffer = msg.getByte() == 1;
			Position pos = msg.getPosition();
			uint16_t spriteId = msg.get<uint16_t>();
			uint8_t stackpos = msg.getByte();
			g_game.playerSendTooltip(player->getID(), pos, spriteId, stackpos, counterOffer);
			break;
		}
		default:
			break;
