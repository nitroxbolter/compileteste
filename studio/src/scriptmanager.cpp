...
bool ScriptingManager::loadScriptSystems()
{
	...
	console::print(CONSOLEMESSAGE_TYPE_STARTUP, "Loading lua libs");
	...
	if (!g_talkActions->loadFromXml()) {
		console::reportFileError(location, "talk actions");
		return false;
	}

	g_moveEvents = new MoveEvents();
	if (!g_moveEvents->loadFromXml()) {
		console::reportFileError(location, "move events");
		return false;
	}

	g_creatureEvents = new CreatureEvents();
	if (!g_creatureEvents->loadFromXml()) {
		console::reportFileError(location, "creature events");
		return false;
	}

	g_globalEvents = new GlobalEvents();
	if (!g_globalEvents->loadFromXml()) {
		console::reportFileError(location, "global events");
		return false;
	}
