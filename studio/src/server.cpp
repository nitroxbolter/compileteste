...
void ServiceManager::stop()
{
	...
	for (auto& servicePortIt : acceptors) {
		try {
			...
			std::string location = "ServiceManager::stop";
			console::reportError(location, fmt::format("Network Error: {}", e.what()));
		}
	}
...
void ServicePort::open(uint16_t port)
{
	...
	try {
		...
		std::string location = "ServicePort::open";
		console::reportError(location, fmt::format("Error: {}", e.what()));
