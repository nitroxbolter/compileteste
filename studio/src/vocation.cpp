	...
	for (auto vocationNode : doc.child("vocations").children()) {
		...
		for (auto attrNode : vocationNode.attributes()) {
			...
			if (strcasecmp(attrName, "name") == 0) {
				...
				warnings.push_back("Unknown attribute: \"" + std::string(attrName) + "\" for vocation: " + std::to_string(voc.id));
			}
		}
			...
			if (strcasecmp(childNode.name(), "skill") == 0) {
				if ((attr = childNode.attribute("id"))) {
					...
					if (skillId <= SKILL_LAST) {
						...
						warnings.push_back("No valid skill id: " + std::to_string(skillId) + " for vocation: " + std::to_string(voc.id));
					}
				} else {
					warnings.push_back("Missing skill id for vocation: " + std::to_string(voc.id));
				}
